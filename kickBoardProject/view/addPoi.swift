...
class PoiView: KakaoMapViewController {
    ...
    func createPoiStyle() {
        ...
        let iconStyle = PoiIconStyle(symbol: UIImage(named: "kickboard.png"), anchorPoint: CGPoint(x: 0.0, y: 0.5))
    ...
    func createPois() {
        ...
        let _ = poi1?.addPoiTappedEventHandler(target: self, handler: PoiView.poiTappedHandler) // poi tap event handler를 추가한다.
        poi1?.show()
    }
